    ...
    return (
        <div>
            ...
            <form onSubmit={addFood}>
                <input type="text" value={foodToAdd} onChange={e=>setFoodToAdd(e.target.value)} placeholder="Food To Add"/>
                <input type="submit" value="Add Food"/>
            </form>
            <ul>
                {food.map(f=>(
                    <li key={f}>{f} <span onClick={e=>removeFood(f)}>X</span></li>
                ))}
            </ul>

            <h1>Guests</h1>
            <form onSubmit={addGuest}>
                <input type="text" value={guestToAdd} onChange={e=>setGuestToAdd(e.target.value)} placeholder="Guest To Add"/>
